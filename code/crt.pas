{
  Modified from the unix version of crt.pas that comes with
  free pascal. Basically, this one file is under the LGPL,
  but with the static linking allowance, allowing it to be
...
{
  textrec. copied in from textrec.inc that comes with fpc.
}
...
var
  ...
  i : longint;
Begin
  if termio.isATTY(F.Handle)=1 then
    begin
      F.BufPos := 0;
      i := 0;
      repeat
